...
function Home(){
    ...
    try{
        const { data } = await axios.get(`https://api.quran.com/api/v4/chapters?language=id/chapters`)
        setchapters(data)
    ...
    const handleSubmit = async (e) => {
        try{
            e.preventDefault()
            const {data} = await axios.get(`https://api.quran.com/api/v4/chapters?language=id/chapter_number=${Search}`)
            // console.log(data, '<== response handle submit')
            setDataSearched(data.chapters )
        }catch (error) {
    ...
    return (
            ...
            <div className="bg-light p-5 rounded">
                <div className="col-sm-8 mx-auto">
                    ...
                    <p>Silahkan mencari surah dengan mengetik angka di menu "Search"</p>
                </div>
